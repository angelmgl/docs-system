        ...
        try {
            ...
            if ($final_url_path) {
                ...
                if (!$stmt->execute()) {
                    ...
                    handle_form_error("No se pudo insertar la imagen número " . ($key + 1) . " en la base de datos.", array(),  "/admin/contenido/documentos/edit_image.php?document_id=" . $document_id);
                }

                $stmt->close();
            } else {
                handle_form_error("No se pudo subir la imagen número " . ($key + 1) . ".", array(),  "/admin/contenido/documentos/edit_image.php?document_id=" . $document_id);
            }
        } catch (Exception $e) {
            handle_form_error("Error con la imagen número " . ($key + 1) . ": " . $e->getMessage(), array(),  "/admin/contenido/documentos/edit_image.php?document_id=" . $document_id);
        }
    }
}
