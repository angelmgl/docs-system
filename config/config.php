<?php 

$db_host = "localhost"; 
$db_user = "root";
$db_password = "";
$db_name = "docs-system";

// Crear la conexión
$mydb = new mysqli($db_host, $db_user, $db_password, $db_name);
$mydb->set_charset('utf8mb4');

// Verificar si hay algún error en la conexión
if ($mydb->connect_error) {
    die("Error en la conexión: " . $mydb->connect_error);
}

define('BASE_URL', 'http://localhost/docs-system');
define('ROOT', __DIR__);
define('MAIL_HOST', 'mail.analytico.com.py');
define('MAIL_USERNAME', 'noreply@analytico.com.py');
define('MAIL_PASSWORD', 'n(*pYiAET8M_');
define('MAIL_FROM', 'noreply@analytico.com.py');
define('APP_NAME', 'Analytico');
define('CONTACT_URL', 'https://wa.me/+595983458839');
define('FILE_MAX_SIZE', 20 * 1024 * 1024); 